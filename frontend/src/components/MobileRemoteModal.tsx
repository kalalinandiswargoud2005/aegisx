...
import { Smartphone, X, Copy, Check, ExternalLink, Wifi, ShieldAlert, Sparkles } from 'lucide-react';
...
export function MobileRemoteModal({ isOpen, onClose }: MobileRemoteModalProps) {
  ...
  // Determine current hostname/IP
  const currentHost = window.location.hostname;
  const currentPort = window.location.port || '5173';
  const remoteUrl = `${window.location.protocol}//${currentHost}:${currentPort}/remote`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-md rounded-2xl bg-gradient-to-b from-[#0e131f] to-[#080b12] border border-cyan-500/40 p-6 shadow-[0_0_50px_rgba(6,182,212,0.25)]">
        ...
        <div className="mb-4 p-3 rounded-xl bg-cyan-950/40 border border-cyan-500/30 text-xs text-gray-300 font-mono flex items-start gap-2">
          ...
          <div>
            <span className="text-cyan-300 font-bold">HOD Presentation Tip:</span> Scan with your phone while connected to the same Wi-Fi. Trigger ransomware and visual HUDs directly from your palm without touching either laptop!
          </div>
        </div>

        {/* QR Code Container */}
        <div className="flex flex-col items-center justify-center p-5 rounded-2xl bg-white/5 border border-white/10 my-4">
          ...
          <p className="text-[11px] font-mono text-gray-400 mt-3 flex items-center gap-1.5">
            Point camera to open <span className="text-cyan-400 font-bold">/remote</span>
          </p>
        </div>

        {/* URL Box */}
        <div className="flex items-center gap-2 bg-black/60 border border-white/10 rounded-xl p-2 mb-5">
          <input
            type="text"
            readOnly
            value={remoteUrl}
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <ExternalLink size={14} /> Open Remote in New Tab
          </a>
          <Button
            variant="outline"
            onClick={onClose}
